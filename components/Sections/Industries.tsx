...
const Industries = () => {
  ...
  return (
    <div id="Industries-section" className="xl:px-[80px] lg:px-[70px] md:px-[16px] px-[16px]">
      <h1 className="xl:text-[48px] lg:text-[38px] md:text-[35px] text-[20px] font-bold md:leading-[50px] leading-[24px] md:tracking-[-2px] tracking-[-1px] text-[#060B13] md:w-[696px] w-[300px] text-center justify-center items-center mx-auto">
        ...
        <div className="grid xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 grid-cols-1 md:gap-8 gap-4 mt-[50px]">
          {Contents.map((content, index) => ( 
            <div
              key={index}
              className="bg-[#F9F5F6] md:rounded-[40px] rounded-[20px] hover:shadow-md transition-shadow duration-300 overflow-hidden flex flex-col h-full"
            >
              <div className="md:pt-8 pt-5 pl-[32px] md:px-7 px-10 flex-grow">
              ...
              <div className="flex justify-center mt-auto h-full min-h-[300px]">
  <Image
    src={content.image}
    alt="icon"
